...
describe('NotesService', () => {
  ...
  beforeEach(() => {
    ...
    } as unknown as jest.Mocked<Pick<PrismaService, 'note'>>;

    service = new NotesService(prisma as unknown as PrismaService);
  });

  it('creates a note owned by the given user', async () => {
    (prisma.note.create as jest.Mock).mockResolvedValue({ id: 'note-1', title: 't', content: 'c', userId: 'user-1' });
    ...
    expect(result).toEqual({ id: 'note-1', title: 't', content: 'c', userId: 'user-1' });
  });
  ...
  it('returns a note the user owns', async () => {
    (prisma.note.findUnique as jest.Mock).mockResolvedValue({ id: 'note-1', userId: 'user-1' });
  ...
  it('throws NotFoundException for a note owned by someone else', async () => {
    (prisma.note.findUnique as jest.Mock).mockResolvedValue({ id: 'note-1', userId: 'someone-else' });

    await expect(service.findOne('user-1', 'note-1')).rejects.toThrow(NotFoundException);
  });
  ...
  it('updates a note only after confirming ownership', async () => {
    (prisma.note.findUnique as jest.Mock).mockResolvedValue({ id: 'note-1', userId: 'user-1' });
    (prisma.note.update as jest.Mock).mockResolvedValue({ id: 'note-1', title: 'updated' });
