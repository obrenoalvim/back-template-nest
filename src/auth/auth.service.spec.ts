...
describe('AuthService', () => {
  let prisma: jest.Mocked<Pick<PrismaService, 'user' | 'verificationToken' | 'passwordResetToken' | '$transaction'>>;
  ...
  beforeEach(() => {
    ...
    } as unknown as jest.Mocked<Pick<PrismaService, 'user' | 'verificationToken' | 'passwordResetToken' | '$transaction'>>;
    ...
    jwtService = { sign: jest.fn().mockReturnValue('signed-jwt') } as unknown as jest.Mocked<JwtService>;

    service = new AuthService(prisma as unknown as PrismaService, emailService, jwtService);
  });
  ...
  describe('register', () => {
    ...
    it('rejects registration when the email is already taken', async () => {
      (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: 'existing', email: 'a@b.com' });
  ...
  describe('validateUser', () => {
    it('returns the user when the password matches', async () => {
      ...
      (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: 'user-1', email: 'a@b.com', password: hashed });
